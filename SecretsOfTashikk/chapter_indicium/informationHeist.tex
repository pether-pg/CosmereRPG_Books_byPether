...
\begin{itemize}
	\item Breaking into the secured area of \locationIndicium{} \\- 0.5 of success per PC
	\item Finding the location of Singer informant \\- 0.5 of success per PC
	...
	\item Finding clues about the Cult's hideout \\- 0.5 of success per PC
	\item Stealing top-secret state information \\- 1 success per PC
\end{itemize} 

The party does not need to define their goals prior to the mission start - the PCs might abandon or pursue new objectives during the mission, depending on how confident do they feel about their progress.

The PCs don't need to be aware of a secret's existence to attempt stealing it. Instead, the PCs might wish to loot the place in search for something useful - each time finding a new secret after gathering 0.5 successes per player.

\reasonBox{
	If the PCs decide to break into \locationIndicium{} before learning that they will need to find the informant's location, encourage the party to search for additional secrets before leaving to make sure they will not get locked from progressing.
}
...
\reasonBox{
	If the party has an odd number of PCs, the partial goals will not be a whole numbers. In that case round them up after summing all of them. Example: if a party of 3 PCs aims to break in, learn about the singer, orphanage and leave safely, they will need to gather 6 successes in total (1.5 + 1.5 + 1.5 + 1.5 = 6). 
}
...
During the Mission, the PCs may attempt one of the below suggestion or propose their own ideas. The DC of the tests should fall in the range of 13-16 (12 + 1d4) at tier 2, or (8 + 1d4 + 2 $\times$ tier) at other tiers.
...
\scriptedOption{Lockpicking.}{
	A PC may attempt a \skillCheck{14}{\skillThievery} to access a room behind a locked door.
}
...
\quoteBox{
	You find a guard report dated for yesterday: "Numerous civilians report a Parsh person in the Immigrant Quarter. White skin, red patterns, dark-blue shiqua. Reports must be false, we failed to find anyone matching the description, and a Parshmen could not outsmart guards on duty."
}
...
\quoteBox{
	You find a piece of document: "With orphanage keeper \npcArshqqam{} departed to Azimir, the institution building is being granted to \locationKharbranth{} surgeons. At the same time, all ongoing investigations about her alleged misuse of supernatural powers are now terminated.
}
...
\quoteBox{
	You find the following note: "It is confirmed that the \locationTashikk{} Prince pushed for the prohibition of supernatural powers while under pressure from the Herald \npcNale{} himself. The Herald works to prevent the return of the Desolations. Can Vorin teachings be false...?"
}

The party learns that the prince of \locationTashikk{} worked with \npcNale{}, the Herald of Justice, to execute all Surgebinders and prevent the return of the Desolations. The recent red storm proves the Herald failed.
...
\begin{itemize}
	\item The emperor is a good friend with a Surgebinder kid, one can expect power-friendly policies from Azir.
...
\ptrBox{
	Risking the mission to steal additional secrets might be recognized by a Cryptic.
}
...
The mission ends when the PCs are captured or after they complete the "Leave \locationIndicium{}" goal. At this point, they cannot proceed with more tests to achieve more objectives.